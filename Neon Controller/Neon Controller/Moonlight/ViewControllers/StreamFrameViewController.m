...
#include "Neon_Controller-Swift.h"
...
@implementation StreamFrameViewController {
    ...
    IBOutlet UIView *controllerOverlayView;
    
    UIView* fullScreenButton;
    UIView* editLayoutButton;

    CGRect fullscreenRect;
    CGRect smallscreenRect;

    BOOL _userIsInteracting;
    
    CGSize _keyboardSize;
    
    id _Nullable changedTab;
...
- (void)applicationFullscreen:(NSNotification *)notification {
    bool isFullscreen = notification.userInfo[@"fullscreen"];
}
...
- (void)viewDidLoad
{
    ...
    [UserData setCurrentFragment: PagesPlay];

    [self.navigationController setNavigationBarHidden:YES animated:YES];
    
    //Changed Tabs Notificaion listener
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(handleChangedTab)
                                                 name: @"ApplicationDidChangeTab"
                                               object: changedTab];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(applicationFullscreen:)
    ...
    // Scaled Down Frame
    double sizePercentage = 0.75;
    smallscreenRect = CGRectMake(
                                        self.view.frame.size.width * ((1 - sizePercentage) / 2),
                                        self.view.frame.size.height * ((1 - sizePercentage) / 2),
                                        self.view.frame.size.width * sizePercentage,
                                        self.view.frame.size.height * sizePercentage);
    
    fullscreenRect = CGRectMake(
                                        0,
                                        0,
                                        self.view.frame.size.width,
                                        self.view.frame.size.height);
    
    _streamView = [[StreamView alloc] initWithFrame: fullscreenRect];
...
//    [_tipLabel setText:@"Tip: Swipe from the left edge to disconnect from your PC"];
    ...
    int index = 0;
    for (ControlButton* controlButton in [[UserData currentController] buttons]) {
        UIButtonView *button = [[UIButtonView alloc] initWithControlButton: controlButton rect: self.view.frame];
        [button addControllerSupport:_controllerSupport];

        [self.view addSubview:button];
        
        index += 1;
        
        if(!UserData.isPremium && index > UserData.defaultMaxButtons) {
            break;
        }
    }
    
    [controllerOverlayView setAlpha:1];
    
    [UserData sendEvent:@"Overlay Created" data:[NSMutableDictionary dictionary]];
    
    [self.view sendSubviewToBack:_streamView];
    
    if(UserData.isPremium && [UserData currentController].isGyro) {
        gyroController = [GyroController alloc];
        [gyroController startGyros:^(NSArray<NSNumber *> * _Nonnull gyroValues) {
            
            double threshold = 0.001 + 0.009 * [UserData currentController].gyroThreshold;
            int sensitivity = 10 + 90 * [UserData currentController].gyroSensitivity;
            
            //        Roll (Y)   Yaw (X)
            if(fabs(gyroValues[1].doubleValue) > threshold || fabs(gyroValues[0].doubleValue) > threshold) {
                
                double deltaX = gyroValues[0].doubleValue * sensitivity;
                double deltaY = -gyroValues[1].doubleValue * sensitivity;
...
- (void)handleChangedTab {
    ...
    NSLog(@"Custom notification received!");
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
    ...
    if(gyroController != nil) {
        [gyroController stopGyros];
    }
...
- (void)connectionTerminated:(int)errorCode {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
            ...
            message = @"Your device's network connection is blocking Neon. Streaming may not work while connected to this network.";
        }
        else {
            switch (errorCode) {
