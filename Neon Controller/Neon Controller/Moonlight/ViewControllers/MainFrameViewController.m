...
@implementation MainFrameViewController {
    ...
    UILabel* backToComputerList;
    UIView* connectionErrorView;
    
    RoundedRect *roundedRect2;
    UILabel* reloadComputerList;
...
- (IBSegueAction UIViewController *)backgroundConstellations:(NSCoder *)coder {
    return [c_AnimatedBackground makeBackground:coder];
}


- (void)startPairing:(NSString *)PIN {
    [UserData sendEvent:@"Began Pairing" data:[NSMutableDictionary dictionary]];
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        self->_pairAlert = [UIAlertController alertControllerWithTitle:@"Pairing"
                                                               message:[NSString stringWithFormat:@"Enter the following PIN on the host machine: %@\n\nIf your host PC is running Neon, navigate to the Neon Desktop App to enter the PIN.", PIN]
...
- (void)pairFailed:(NSString *)message {
    [UserData sendEvent:@"Pairing Failed" data:[NSMutableDictionary dictionary]];
...
- (void)pairSuccessful:(NSData*)serverCert {
    [UserData sendEvent:@"Pairing Successful" data:[NSMutableDictionary dictionary]];
...
- (void)backToList:(UITapGestureRecognizer *)gestureRecognizer {
    // Handle the tap here
    [self showHostSelectionView];
    
    if (@available(iOS 10.0, *)) {
        // Create a feedback generator for the default (generic) haptic feedback
...
- (void)alreadyPaired {
    [UserData setCurrentFragment: PagesGames];
    ...
    if(roundedRect == nil && backToComputerList == nil) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            
            CGRect frameForAll = CGRectMake(48, [[self view] frame].size.height - 48, 144, 32);
            
            self->roundedRect = [[RoundedRect alloc] init:frameForAll radius:0 backgroundColor:[UIColor colorNamed: @"Primary1"]];
            [self.view addSubview:self->roundedRect];
            
            
            self->backToComputerList = [[UILabel alloc] initWithFrame: frameForAll];
            
            [self->backToComputerList setText:@"<- Back To Computers"];
            
            [self->backToComputerList setTextColor: [UIColor blackColor]];
            
            [self->backToComputerList setTextAlignment: NSTextAlignmentCenter];
            
            [self->backToComputerList setFont: [UIFont fontWithName: @"Oswald-Medium" size: 12]];
            
            UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(wahoo:)];
            [self->roundedRect addGestureRecognizer:tapGesture];
            
            [[self view] addSubview:backToComputerList];
...
- (void)showHostSelectionView {
    ...
    [UserData setCurrentFragment: PagesComputers];
    ...
    if(backToComputerList != nil) {
        [backToComputerList removeFromSuperview];
        backToComputerList = nil;
    }
    ...
    NSLog(@"63b %d computers", hostScrollView.subviews.count);
}
...
- (void) hostClicked:(TemporaryHost *)host view:(UIView *)view {
    ...
    if(reloadComputerList != nil) {
        [reloadComputerList removeFromSuperview];
        reloadComputerList = nil;
    }
    
    if(roundedRect2 != nil) {
        [roundedRect2 removeFromSuperview];
        roundedRect2 = nil;
    }
...
//                            message = [NSString stringWithFormat:@"Your current network connection seems to be blocking Neon. Streaming may not work while connected to this network.\n\nThe following network ports were blocked:\n%s", blockedPorts];
//                        }
//
...
//        }]];
    ...
    [longClickAlert addAction:[UIAlertAction actionWithTitle:@"Remove Host" style:UIAlertActionStyleDestructive handler:^(UIAlertAction* action) {
        ...
        @synchronized(hostList) {
            ...
            if([hostList count] == 0) {
                [self createConnectionErrorTextView];
            }
        }
...
- (void) addHostClicked {
    ...
    UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Add Host Manually" message:@"If Neon doesn't find your local gaming PC automatically,\nenter the IP address of your PC" preferredStyle:UIAlertControllerStyleAlert];
                ...
                [self->_discMan discoverHost:hostAddress withCallback:^(TemporaryHost* host, NSString* error){
                    if (host != nil) {
                        ...
                        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
                            error = [error stringByAppendingString:@"\n\nYour device's network connection is blocking Neon. Streaming may not work while connected to this network."];
                        }
...
- (void) appClicked:(TemporaryApp *)app view:(UIView *)view {
    ...
    [UserData sendEvent:@"App Clicked" data:[NSMutableDictionary dictionary]];
...
//    [self.view addGestureRecognizer:self.revealViewController.panGestureRecognizer];
    ...
    // Makes the background transparent for constellations
    ...
    hostScrollView.frame = CGRectMake(0, self.navigationController.navigationBar.frame.origin.y + self.navigationController.navigationBar.frame.size.height + 40, self.view.frame.size.width, self.view.frame.size.height / 2);
...
-(void)createReloadButton {
    ...
    self->roundedRect2 = [[RoundedRect alloc] init:frameForAll radius:0 backgroundColor:[UIColor colorNamed: @"Primary1"]];
    [self.view addSubview:self->roundedRect2];
    
    
    self->reloadComputerList = [[UILabel alloc] initWithFrame: frameForAll];
    
    [self->reloadComputerList setText:@"Reload Connections"];
    
    [self->reloadComputerList setTextColor: [UIColor blackColor]];
    
    [self->reloadComputerList setTextAlignment: NSTextAlignmentCenter];
    
    [self->reloadComputerList setFont: [UIFont fontWithName: @"Oswald-Medium" size: 12]];
    
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(refreshComputerList)];
    [self->roundedRect2 addGestureRecognizer:tapGesture];
    
    [[self view] addSubview:reloadComputerList];
}


-(void)createConnectionErrorTextView {

//    if(connectionErrorView == nil) {
        
        //Put in tutorial view
        CGFloat centerX = CGRectGetMidX(self.view.bounds);
        CGFloat centerY = CGRectGetMidY(self.view.bounds);
        
        UIView* container = [[UIView alloc] initWithFrame:[self.view bounds]];
        
        UIImage* errorImage = [UIImage imageNamed:@"info_icon"];
        UIImage* errorImageColored = [errorImage imageWithTintColor:[UIColor redColor]];
        UIImageView *imageView = [[UIImageView alloc] initWithImage: errorImageColored];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        
        int padding = 8;
        int imageSize = 48;
        [imageView setFrame:CGRectMake(centerX - imageSize / 2, 0, imageSize, imageSize)];
        
        int textTitleLabelSize = 300;
        UILabel* titleTextLabel = [[UILabel alloc] init];
        titleTextLabel.text = @"Connection Error";
        titleTextLabel.textColor = [UIColor whiteColor];
        titleTextLabel.font = [UIFont fontWithName:@"Oswald-Medium" size:32];
        titleTextLabel.textAlignment = NSTextAlignmentCenter;
        [titleTextLabel setFrame:CGRectMake(centerX - textTitleLabelSize / 2, CGRectGetMaxY(imageView.frame) + padding, textTitleLabelSize, 32)];
        
        
        int textLabelSize = 200;
        UILabel* bodyTextLabel = [[UILabel alloc] init];
        bodyTextLabel.text = @"Download the host software on your\ncomputer from neoncontroller.com\nto play";
        bodyTextLabel.textColor = [UIColor whiteColor];
        bodyTextLabel.font = [UIFont fontWithName:@"OpenSans-Light" size:16];
        ...
        [bodyTextLabel setFrame:CGRectMake(centerX - textTitleLabelSize / 2, CGRectGetMaxY(titleTextLabel.frame), textTitleLabelSize, 72)];
        
        [container addSubview:bodyTextLabel];
        [container addSubview:titleTextLabel];
        [container addSubview:imageView];
        
        int totalHeight = bodyTextLabel.frame.size.height +
        titleTextLabel.frame.size.height +
        imageView.frame.size.height +
        padding;
        
        [container setFrame:CGRectMake(
                                       centerX - container.frame.size.width / 2,
                                       centerY - totalHeight / 2,
                                       container.frame.size.width,
                                       totalHeight)];
        
        
        connectionErrorView = container;
