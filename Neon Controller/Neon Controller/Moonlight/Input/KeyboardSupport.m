...
+ (void) mSendKeyEvent:(unichar)inputChar isDown:(BOOL) down {
    ...
    struct KeyEvent key = [self translateKeyEvent:inputChar withModifierFlags:0];
    
    LiSendKeyboardEvent(0x8000 | key.keycode,
                        down ? KEY_ACTION_DOWN : KEY_ACTION_UP,
                        0);
    
}

+ (void) mSendKeyEventAction:(u_short)key isDown:(BOOL) down {
    ...
    u_short keyCode;
        ...
        switch (key) {
            ...
            case UIKeyboardHIDUsageKeyboardRightAlt:
                ...
                break;
            default:
                NSLog(@"Unhandled HID usage: %lu", (unsigned long) keyCode);
                assert(0);
                return;
