...
-(void) sendButtonKeycode:(Controller*)controller buttonKeyCode:(int) buttonKeyCode down:(bool) down {
    
    NSNumber* buttonIDID = [controllerButtonList objectAtIndex:buttonKeyCode];
    int buttonID = buttonIDID.intValue;
    
    if(down) {
        [self setButtonFlag:controller flags:buttonID];
    } else {
        [self clearButtonFlag:controller flags:buttonID];
    }
...
-(id) initWithConfig:(StreamConfiguration*)streamConfig delegate:(id<ControllerSupportDelegate>)delegate
{
    ...
    controllerButtonList = [[NSArray alloc] init];
    controllerButtonList = @[@UP_FLAG, @DOWN_FLAG, @LEFT_FLAG, @RIGHT_FLAG, @PLAY_FLAG, @BACK_FLAG, @LS_CLK_FLAG, @RS_CLK_FLAG, @LB_FLAG, @RB_FLAG, @A_FLAG, @B_FLAG, @X_FLAG, @Y_FLAG, @0, @0, @RB_FLAG, @LB_FLAG, @SPECIAL_FLAG];
